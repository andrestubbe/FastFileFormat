...
javac -cp "%CP%" -d "examples\Demo\target\classes" "examples\Demo\src\main\java\fastfileformat\demo\Demo.java"
if %ERRORLEVEL% NEQ 0 ( echo ❌ Demo compile failed. & pause & exit /b %ERRORLEVEL% )

echo 🚀 Running FastFileFormat Interactive Demo...
java -cp "%CP%;examples\Demo\target\classes" fastfileformat.demo.Demo
